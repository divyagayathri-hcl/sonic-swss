...
class DBInterface(object):
    ...
    def set_up_databases(self, dvs):
        ...
        # 1. Use shell echo to bypass Python's buffer and ensure log visibility
        os.system(f"echo 'DEBUG: Starting set_up_databases'")
        os.system(f"echo 'DEBUG: ZMQ Socket Path: {dvs.p4orch_zmq_sock}'")

        # 2. Check for socket existence via shell
        os.system(f"[ -e {dvs.p4orch_zmq_sock} ] && echo 'DEBUG: ZMQ socket exists' || echo 'DEBUG: ZMQ socket MISSING'")

        # 3. Check for the Redis socket (usually the real cause of Aborted errors)
        os.system("[ -S /var/run/redis/redis.sock ] && echo 'DEBUG: Redis socket exists' || echo 'DEBUG: Redis socket MISSING'")

        self.appl_state_db = _set_up_appl_state_db(dvs)

        # To print dvs.p4orch_zmq_sock
        print(f"ZMQ Socket Path: {dvs.p4orch_zmq_sock}")
        if os.path.exists(dvs.p4orch_zmq_sock):
